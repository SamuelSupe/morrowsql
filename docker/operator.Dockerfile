...
RUN --mount=type=secret,id=build_ca,target=/run/secrets/build-ca.pem,mode=0444 \
    ...
    && apt-get install -y --no-install-recommends libpython3.12t64 libssh-4 libedit2 python3-pip locales \
    && localedef -i en_US -f UTF-8 en_US.UTF-8 \
    && rm -rf /var/lib/apt/lists/* /etc/apt/apt.conf.d/99build-ca \
    && mkdir /mysqlsh && chown 2 /mysqlsh
COPY docker/operator-requirements.lock /tmp/operator-requirements.txt
RUN --mount=type=secret,id=build_ca,target=/run/secrets/build-ca.pem,mode=0444 \
    PIP_CERT="$(if [ -s /run/secrets/build-ca.pem ]; then echo /run/secrets/build-ca.pem; else echo /etc/ssl/certs/ca-certificates.crt; fi)" \
    pip3 install --no-cache-dir --require-hashes --target=/opt/operator-deps -r /tmp/operator-requirements.txt \
...
# A client socket in the standalone server configuration makes mysqlsh try to
# connect before starting the controller. Sidecars mount their own configuration.
RUN rm /etc/my.cnf
